...
pub(crate) fn denali_to_drtrs_address(denali_address: &AddressValue) -> ErdrsAddress {
    let bytes = denali_address.value.as_array();
    ErdrsAddress::from_bytes(*bytes)
}

pub(crate) fn address_h256_to_drtrs(address: &Address) -> ErdrsAddress {
    let bytes = address.as_array();
    ErdrsAddress::from_bytes(*bytes)
}

pub(crate) fn drtrs_address_to_h256(drtrs_address: ErdrsAddress) -> Address {
